...
from magick_mind.memories.semantic_memory.dto.semantic_memory_dto import SemanticMemoryDTO
...
class SemanticMemory:
    ...
    async def build(self):
        ...
        self.core_knowledge, self.domain_specific_knowledge, \
            self.company_knowledge, self.personal_knowledge = await asyncio.gather(
    ...
    async def recall(self, user_query: str) -> SemanticMemoryDTO:
        domain_knowledge, core_knowledge, company_knowledge, personal_knowledge = await asyncio.gather(
            self.domain_specific_knowledge.aquery(
                user_query, param=QueryParam(mode="hybrid")),
            self.core_knowledge.aquery(
                user_query, param=QueryParam(mode="hybrid")),
            self.company_knowledge.aquery(
                user_query, param=QueryParam(mode="hybrid")),
            self.personal_knowledge.aquery(
                user_query, param=QueryParam(mode="hybrid"))
        )
