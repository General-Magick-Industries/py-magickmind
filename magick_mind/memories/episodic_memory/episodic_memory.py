...
db = mongo_client.get_database('agentic_memory')
collection = db.get_collection('episodic_memory')

if 'episodic_memory' not in db.list_collection_names():
    db.create_collection('episodic_memory')
...
def reflect(conversation: str) -> dict:
    reflection_prompt = f"""
...
def store_memory(reflection: dict, conversation: str):
...
def recall(query: str) -> Optional[Dict]:
...
def get_previous_day_conversation(selected_date: datetime) -> str:
...
def get_previous_month_conversation(selected_date: datetime) -> str:
...
def get_previous_year_conversation(selected_date: datetime) -> str:
    """Get conversation summaries for a specific year"""
    start_date = selected_date.replace(day=1, month=1)
    end_date = (selected_date.replace(day=1, month=1) + timedelta(days=365)).replace(day=1, month=1)
...
def get_episodic_memory(query: str) -> Optional[Dict]:
    previous_day_conversation = get_previous_day_conversation(datetime.now())
    previous_month_conversation = get_previous_month_conversation(datetime.now())
    previous_year_conversation = get_previous_year_conversation(datetime.now())

    similar_conversations = recall(query)
    
    return f"""Previous day conversation:
    {previous_day_conversation}\n\n
    Previous month conversation:
    {previous_month_conversation}\n\n
    Previous year conversation:
    {previous_year_conversation}\n\n
