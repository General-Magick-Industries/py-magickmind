...
class EmailPasswordAuth(AuthProvider):
    ...
    def get_headers(self) -> Dict[str, str]:
        ...
        if not self._access_token:
            raise AuthenticationError("Not authenticated. Failed to obtain access token.")
    ...
    def _store_tokens(self, token_data: TokenResponse) -> None:
        ...
        expires_in = token_data.get("expires_in", 300) - 10
        self._token_expires_at = current_time + max(expires_in, 0)
        
        refresh_expires_in = token_data.get("refresh_expires_in", 1800) - 10
